...
json_data = [
    {"MFMLIN": 10, "MATRATCALC": 17, "PRIORITY": True, "POSITION": "A"},
...
def main():
    ...
    results = []
    for s in top:
        # 使用 MFMLIN 的值作为分配结果的键
        assign_result = {
            buckets[i]: json_data[af]['MFMLIN'] 
            for i, af in enumerate(s['assign'])
        }
        results.append({
            'X1': round(s['X1'], 4),
            'X2': round(s['X2'], 4),
            ...
            'assign': assign_result,  # 直接使用 MFMLIN 的值
